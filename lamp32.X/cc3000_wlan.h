...
typedef struct _ap_entry_t {
		uns8 entry_count;	//< Number of this entry point (zero for last)
		uns8 scan_status;	//< Status of the scan 0 = aged results, 1 = results valid, 2 = no result
		uns8 entry_valid;	//< Validity of the entry, 0 = invalid, 1 = valid
		uns8 rssi;			//< RSSI entry
		uns8 security;		//< Secutity type, 0 = Open, 1 = WEP, 2 = WPA, 3 = WPA2
		uns8 ssid[32 + 1];	//< SSID name
} ap_entry_t;
...
hci_status_t cc3000_wlan_set_scan_params(uns16 scan_frequency, uns16 channel_mask);
...
/**
	...
	\param Event Mask. A bit that is 1 = mask event
