...
void FSGetDiskProperties(FS_DISK_PROPERTIES* properties)
{
    ...
    #ifdef SUPPORT_FAT32 // If FAT32 supported.
            case FAT32:
                properties->_private.EndClusterLimit = END_CLUSTER_FAT32;
                properties->_private.ClusterFailValue = CLUSTER_FAIL_FAT32;
                break;
    #endif
            case FAT16:
                properties->_private.EndClusterLimit = END_CLUSTER_FAT16;
                properties->_private.ClusterFailValue = CLUSTER_FAIL_FAT16;
                break;
            case FAT12:
                properties->_private.EndClusterLimit = END_CLUSTER_FAT12;
                properties->_private.ClusterFailValue = CLUSTER_FAIL_FAT16;
                break;
        }
    
        properties->_private.c = 2;

        properties->_private.curcls = properties->_private.c;
        ReadFAT(properties->disk, properties->_private.c);
    }
    ...
    {
        // look at its value
        if ( (value = ReadFAT(properties->disk, properties->_private.c)) == properties->_private.ClusterFailValue)
        {
        ...
        properties->_private.c++;    // check next cluster in FAT
        // check if reached last cluster in FAT, re-start from top
        if (value == properties->_private.EndClusterLimit || properties->_private.c >= (properties->results.total_clusters + 2))
            properties->_private.c = 2;

        // check if full circle done, disk full
        if ( properties->_private.c == properties->_private.curcls)
        {
